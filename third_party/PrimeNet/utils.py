...
def torch_load(path, map_location=None):
    """Load checkpoints saved with torch.save (PyTorch 2.6+ needs weights_only=False)."""
    kwargs = {}
...
def get_data_min_max(records, device):
    """Per-feature min/max over observed values (PhysioNet / Activity tuples)."""
    ...
    for b in records:
        ...
        if data_min is None:
            data_min = torch.full((n_features,), inf).to(device)
            data_max = torch.full((n_features,), -inf).to(device)
        for k in range(n_features):
            data_k = vals[:, k][mask[:, k] == 1]
            if len(data_k) > 0:
                data_min[k] = min(data_min[k], data_k.min())
                data_max[k] = max(data_max[k], data_k.max())
    return data_min, data_max



def variable_time_collate_fn(batch, device, classify=False, data_min=None, data_max=None,
                             activity=False):
    """Pad irregular samples to [batch, seq_len, 2*D+1] (values, mask, time)."""
    del activity  # kept for API compatibility with preprocess_activity.py
    batch_size = len(batch)
    n_features = batch[0][2].size(-1)

    combined_vals, combined_masks, combined_times, combined_labels = [], [], [], []
    for _, tt, vals, mask, labels in batch:
        vals = vals.to(device)
        mask = mask.to(device)
        tt = tt.to(device)
        if data_min is not None and data_max is not None:
            vals, _, _ = normalize_masked_data(vals, mask, data_min, data_max)
        combined_vals.append(vals)
        combined_masks.append(mask)
        combined_times.append(tt)
        if classify:
            combined_labels.append(labels)

    max_len = max(v.size(0) for v in combined_vals)
    padded_vals = torch.zeros(batch_size, max_len, n_features, device=device)
    padded_masks = torch.zeros(batch_size, max_len, n_features, device=device)
    padded_times = torch.zeros(batch_size, max_len, device=device)

    for i in range(batch_size):
        seq_len = combined_vals[i].size(0)
        padded_vals[i, :seq_len] = combined_vals[i]
        padded_masks[i, :seq_len] = combined_masks[i]
        padded_times[i, :seq_len] = combined_times[i]

    time_dim = padded_times.unsqueeze(-1)
    combined_data = torch.cat([padded_vals, padded_masks, time_dim], dim=-1)

    if classify:
        labels = torch.stack(
            [torch.tensor(l, device=device) for l in combined_labels]
        )
...
def generate_batches(X_train, X_val, args):
    ...
    train_dataloader = DataLoader(pretrain_data, batch_size = batch_size, shuffle = True, collate_fn = train_cl_collator, num_workers=0)
    val_dataloader = DataLoader(val_data, batch_size = batch_size, shuffle = False, collate_fn = train_cl_collator, num_workers=0)
